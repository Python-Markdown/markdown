...
A block processor, also called a block parser, works with blocks of text for complex parsing
and adds new elements to the `ElementTree`.   The block, a chunk of lines separated from other
text by blank lines, may have a different syntax and produce a differently structured tree than
...
and implement both the `test` and `run` methods.  `test` returns `True` if the 
block processor's `run` method should be called with a block.
`test(self, parent, block)` takes two parameters: `parent` is the parent `ElementTree` element and `block` 
is a single, multi-line, Unicode string with the current block.   The `test` function is
often a simple regular expression.  `run(parent, blocks)` has the same `parent` parameter and
`blocks` is a list of all remaining blocks in the document.  `run` may return `False` to 
signal it did not process the block after all.    Each `run` invocation is expected to pop 
one or more blocks from the front of the `blocks` andattach new nodes to its `parent`.

Crafting block processors is more involved and flexible than the other processors, involving 
controlling recursive parsing of the block's contents and managing state across invocations.  
For example, a blank line is allowed in indented code, so the second invocation of the inline 
code processor appends to the element tree generated by the previous call.  Other block
processors may insert new text into the `blocks` list, signal to future calls of itself, and
more.

To make writing these complex beasts more tractable, a number of convenience functions have been
provided in the `BlockProcessor` parent class.  `lastChild(parent)` returns the last child of 
the given element or `None` if it has no children.  `detab(text)` removes one level of indent 
(four spaces by default) from the front of each line of the given text string.  
`looseDetab(text, level)` removes multiple levels of indent from the front of each line of `text` but
does not affect lines indented less.  Also, `BlockProcessor` provides the fields `self.tab_length`, 
the tab length (default 4), and `self.parser`, the current `BlockParser` instance.  

`BlockParser`, not to be confused with `BlockProcessor`, is the code that cycles through all
the block processors, parsing the markdown and generating the `ElementTree`.   It provides a 
...
Be sure your code pops and states it pushes.  

The parser can also be called recursively to parse up text, e.g., to process 
Markup within a block of text.  You can call `parseDocument(lines)` to pares
a list of lines, each a single-line Unicode string. 
`parseChunk(parent, text)` parses a single, multi-line, Unicode string `text`
and attaches the resulting tree to `parent`.  `parseBlocks(parent, blocks)` takes
a list of `blocks`, each a multi-line Unicode string, and attaches the resulting
tree to `parent`.   For perspective, Markdown calls `parseDocument` which calls
`parseChunk` which calls `parseBlocks` which calls your block processor, which
could call `parseChunk`. 
...
We create a block processor that triggers on a block starting with `!!!` and processes blocks until
it sees another `!!!`

.... of to write code in the morning ...
