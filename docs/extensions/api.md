...
A tree processor must inherit from `markdown.treeprocessors.TreeProcessor`. A tree processor must implement a `run`
method which takes a single argument `root`. In most cases `root` would be an `xml.etree.ElementTree.Element` instance;
