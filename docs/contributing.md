...
!!! seealso "See Also"
    ...
    This should link to some relevent content contained elsewhere.
```
...
The above codeblock will be rendered at follows:
...
block, then they can be defined as YAML frontmatter within the code block.
The `extensions` and `extension_configs` keywords are the only options which
are passed on to the isolated Markdown instance.The YAML frontmatter is
stripped from the body of the code block before the codeblock is passing to
...
Note that the YAML frontmatter must be deliminated by a line of at least three
...
proposed changes which utailize rendered code blocks.
